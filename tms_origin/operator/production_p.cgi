...
my @file = ();
my @index = ();
&TMScommon::get_operator_file_list( $period, \@day, \@file, \@index );	# 期間の条件にマッチするファイル抽出
...
# 各ファイル名の取得
my $xlsfile  = "../xlsfile/$lang/production.xls";

my $datafile = &TMScommon::get_xlsdata_file_name("production","csv");

my $infofile = "production.tmshlp";
...
print OUT &make_production_title( 'operator', $period, $ttl_unit );
  ...
  while(<IN>){
    ...
    if( $line =~ m/,ope_name ([^,]+)/ ){
      if( exists($operator{$1}) ){
        ...
        $dcount++;
        print OUT &make_production_data( 'operator', $index[$i], \$line, $val_unit );
