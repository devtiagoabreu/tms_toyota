...
if( &TMScollect::check_collect_date() ){  # 最近、データ収集したか
  ...
  if( $extend eq "on" ){  # 強引に進むか？
    ...
    my $url = "select_s.cgi?mode=$mode&period=$period&extend=on";
...
if( &TMSrestruct::check_restruction() ){  # 再構築が必要の場合
  ...
  if( $force ne "on" ){  # 強引に進むか？
    ...
    my $url = "select_s.cgi?mode=$mode&period=$period&force=on";
...
if( $mode eq 'shiftreport' ){
	$title = $str_SHIFT_REPORT;
	$cgifile = 'shiftreport_s.cgi';
	$submit_button = $str_SHOW_REPORT;
}
elsif( $mode eq 'stylereport' ){
	$title = $str_STYLE_REPORT;
	$cgifile = 'stylereport.cgi';
	$submit_button = $str_SHOW_REPORT;
}
elsif( $mode eq 'statushistory' ){
	$title = $str_STATUS_HISTORY;
	$cgifile = 'statushistory.cgi';
	$submit_button = $str_SHOW_REPORT;
}
elsif( $mode eq 'production' ){
	$title = $str_PRODUCTION_GRAPH;
	$cgifile = 'production_s.cgi';
	$submit_button = $str_SHOW_GRAPH;
}
elsif( $mode eq 'efficiency' ){
	$title = $str_EFFICIENCY_GRAPH;
	$cgifile = 'efficiency_s.cgi';
	$submit_button = $str_SHOW_GRAPH;
}
elsif( $mode eq 'stopanalysis' ){
	$title = $str_STOP_ANALYSIS_GRAPH;
	$cgifile = 'stopanalysis.cgi';
	$submit_button = $str_SHOW_GRAPH;
}
elsif( $mode eq 'forecast' ){
	$title = $str_YARN_INVENTORY_FORECAST;
	$cgifile = 'forecast.cgi';
	$submit_button = $str_SHOW_REPORT;

	$loom_list  = '../../tmsdata/index/current_loom.txt';
	$style_list = '../../tmsdata/index/current_style.txt';
	$sel_shift = 0;
	$colspan = 2;
}
...
#	"  }\n".
	...
	"    location.href = \"select_s.cgi?mode=$mode&period=\" + period;\n".
	"  }\n".
	"//-->\n".
	"</script>\n".
	"</head>\n".
