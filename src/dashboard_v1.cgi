...
foreach my $loom (@loom_id_name) {
  ...
  my $weft_stops = 0;
  my $warp_stops = 0;
  my $other_stops = $stop_shift;
  
  if( $status eq 'Weft' ) {
    $weft_stops = int($stop_shift * 0.7);
    $warp_stops = int($stop_shift * 0.2);
    $other_stops = $stop_shift - $weft_stops - $warp_stops;
  }
  elsif( $status eq 'Warp' || $status eq 'Warp_top' ) {
    $warp_stops = int($stop_shift * 0.7);
    $weft_stops = int($stop_shift * 0.2);
    $other_stops = $stop_shift - $weft_stops - $warp_stops;
  }
  else {
    $weft_stops = int($stop_shift * 0.4);
    $warp_stops = int($stop_shift * 0.4);
    $other_stops = $stop_shift - $weft_stops - $warp_stops;
  }
