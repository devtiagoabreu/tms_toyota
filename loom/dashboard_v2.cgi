...
print	"<html lang=$lang>\n".
	...
	"  border-radius: 8px;\n".
	"}\n".
	".dark .metric { background: #2a2a2a; }\n".
	".light .metric { background: #f8f9fa; border: 1px solid #e9ecef; }\n".
	".metric-value { \n".
	"  font-size: 24px; \n".
	"  font-weight: bold; \n".
	"  margin-bottom: 4px;\n".
...
foreach my $loom (@loom_id_name) {
  ...
  # Determinar cor do status para a borda
  ...
  print "<div class=\"card\" style=\"border-color: $status_color\">\n".
...
print "</div>\n".
      ...
      "  \n".
      "  // Atualiza a página com o novo tema\n".
      ...
      "  // O CSS Grid já é responsivo, mas podemos adicionar lógica adicional se necessário\n".
...
{
  # Configurações dos teares do scanner
  my @loom_set = &TMSscanner::get_loom_setting( "mac_name" );

  # Se o nome do tear não estiver definido, usar o ID
  foreach(@loom_set){
...
sub get_scan_loom_status
{
  ...
  # Definir status de erro para teares sem dados
  foreach my $mac_id ( @$r_mac_id ){
